...
| **Auth Gateway** (Starlette) | Authenticate, authorize (org membership), host the sign-in / request-access surfaces, inject identity headers, reverse-proxy to Streamlit incl. WebSockets, and re-validate open sockets via heartbeat. Implemented across `gateway/`: `auth_gateway.py` (access decision + identity headers), `proxy.py` (HTTP/WS reverse proxy), `ws_revalidation.py` (heartbeat + connection registry + sweeper), `clerk_auth.py` (verify), `streamlit_config.py` (headless/proxy config). | AuthProvider, Streamlit process |
